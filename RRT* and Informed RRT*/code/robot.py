...
class Robot:
    ...
    def check_collision(
        ...
        configs_between = self.interpolate(config1, config2, num=25)
    ...
    def check_collision_config(
        ...
        ### YOUR CODE HERE ###
        if len(config) == 3: # Checking is the omnidir robot endpoints collide with obstacles
            for c in self.forward_kinematics(config)[:4]:
                for obstacle in obstacles:
